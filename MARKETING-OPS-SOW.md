...
> Tài liệu chính thức về vai trò, phạm vi trách nhiệm, quy tắc phối hợp và workflow vận hành cho **Marketing Operations Manager** — trung tâm điều phối và ra quyết định giữa AM/Pub, GEO, Finance, Regulatory Ops và Product.
...
**Marketing Operations Manager** = người **quản lý vận hành + ra quyết định** ở giao điểm của **5 trục**:
...
- **LEAD** team Ops (ICs) và cross-team alignment

### 1.2. Manager vs IC — Ranh giới rõ ràng

| Hạng mục | IC làm (Individual Contributor) | **Manager làm** |
|---|---|---|
...
> **Nguyên tắc vàng:** *Manager không làm việc IC. Manager xây hệ thống để IC làm việc hiệu quả và can thiệp khi có exception.*

### 1.3. Triết lý cốt lõi

> *"MKT Ops Manager tồn tại để các team khác không phải tự đi tìm vấn đề, không phải tự định nghĩa SOP, không phải tự align lẫn nhau. Manager là người xây hệ thống chạy tự động ở mức IC — và là người ra quyết định cuối cùng khi có exception."*
...
```
1. IC compile offers AM đang hỏi + offers GEO đang push
2. IC scorecard mỗi offer: expected CR, payout, margin, risk, compliance
3. MANAGER decide: Go / No-Go / Test phase
4. MANAGER align với AM + GEO về launch timeline
5. IC execute launch workflow (WF-4)
```
...
> *Standardize cross-department communication / workflow / SOP / reports across AM, GEOs, FN, Reg. Ops.*
...
| **SOP-04** | Reg. Ops ↔ Ops compliance check (new market, new offer) |
...
#### 🎯 MANAGER OWNS
- **SET** target cost & volume theo quý (ngân sách)
- **APPROVE** bất kỳ thay đổi CPL nào vượt **±10%** so với baseline
- **NEGOTIATE** với AM khi cost lệch target
- **DECIDE** pub/GEO nào scale, pub/GEO nào cut

#### Team executes
- Daily tracking: CPL theo GEO/Category/Pub/Deal
- Weekly variance report
- Flag outliers cho Manager

#### Manager Dashboard (track daily)
| Metric | Target | Threshold alert |
|---|---|---|
| Avg CPL theo GEO | Set per quarter | ±20% |
| Daily lead volume | Set per quarter | ±30% |
| Cost per approved lead | GEO baseline | +25% |
| Rejection rate | <5% | >10% |

#### Rules
- CPL >target **+20%** 3 ngày liên tiếp → IC alert Manager, Manager decide trong 24h
- Volume <target **-30%** tuần → Manager review cause với AM
...
#### KPI Manager
- CPL variance vs target <15% quarterly
...
|---|---|---|
| AR% drop >20% trong 3 ngày | Cap down 30% | IC propose, **Manager approve** |
| Stock <10 ngày | Cap down 50% | IC execute ngay, notify Manager |
| CR ↑ >30% ổn định 5 ngày | Cap up 20% | IC propose, **Manager approve** |
| GEO SOS | Cap down 70% | IC execute ngay, notify Manager |
...
| Pub request scale <20% | Cap up | IC execute, notify Manager |

#### Rules
- Mọi cap change phải có **số liệu backing + thời gian review**
- Cap up **>50%** bắt buộc align trước với GEO (capacity confirm)
- Cap down khẩn cấp: IC execute trước, Manager notified trong 1h
...
- Partner S tier có direct line với Manager — không qua IC
...
- Career development plan cho từng IC
...
- Build bridge giữa các department head: AM Head, GEO Head, FN Head, Reg. Ops Head, Product Head
...
### 4.4. Với REG. OPS ⭐ (stakeholder quan trọng thường bị bỏ qua)
**Họ cần:** biết sớm offer sắp launch, compliance risks, flag kịp để không block.

**Rules:**
1. New offer → align Reg. Ops **tối thiểu 7 ngày** trước launch
2. New GEO → compliance check trước mọi activity
3. Reg. Ops flag vấn đề → **Manager respond trong 2h**, không ignore
4. Compliance training **yearly** cho team Ops
5. LDP claim changes → Reg. Ops sign-off trước khi live
...
9. **Manager doesn't do IC work.** Manager caught doing IC work = team đang thiếu người hoặc Manager không trust team.
...
2. **Install Rule 20%:** mọi metric lệch 20% → IC alert trong 30 phút
